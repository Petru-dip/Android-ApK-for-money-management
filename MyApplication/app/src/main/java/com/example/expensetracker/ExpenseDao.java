...
public interface ExpenseDao {
    ...
    @Query("DELETE FROM Expense WHERE id = :id") void deleteById(int id);
    @Query("SELECT * FROM Expense WHERE id = :id LIMIT 1") Expense getById(int id);
    @Query("SELECT * FROM Expense WHERE uid = :uid LIMIT 1") Expense getByUid(String uid);
    @Query("SELECT * FROM Expense WHERE uid IS NULL OR uid = ''") List<Expense> getMissingUid();

    @Query("SELECT * FROM Expense ORDER BY date DESC") List<Expense> getAll();
    ...
    @Query("SELECT SUM(amount) FROM Expense WHERE category = :category AND date BETWEEN :from AND :to")
    Double getTotalByCategory(String category, long from, long to);
}
